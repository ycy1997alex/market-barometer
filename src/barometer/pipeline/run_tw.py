"""台股每日排程進入點（ToDo §9 Day 24 第 10 項）。

Windows 工作排程器每天 18:00 呼叫這支。**不是 GitHub Actions cron** ——
...
def main() -> int:
    log = fetch_prices.run(
        list(config.TW_SYMBOLS), task="daily_tw", period="1y"
    )
    print(f"[{log.started_at:%Y-%m-%d %H:%M}] daily_tw "
